...
const defaultForm: NotifForm = { showVersion: true, ndaText: '', forceOnVersionChange: false };
...
function Toggle({ on, onChange, locked }: { on: boolean; onChange: () => void; locked?: boolean }) {
  return (
    <div onClick={locked ? undefined : onChange}
      style={{ width: 36, height: 20, borderRadius: 10, background: on ? 'var(--accent)' : 'var(--bg-overlay)', border: '1px solid var(--border)', position: 'relative', cursor: locked ? 'not-allowed' : 'pointer', transition: 'background 0.2s', flexShrink: 0, opacity: locked ? 0.6 : 1 }}>
      <span style={{ position: 'absolute', top: 2, left: on ? 18 : 2, width: 14, height: 14, borderRadius: '50%', background: on ? 'white' : 'var(--text-muted)', transition: 'left 0.2s' }} />
    </div>
  );
}
...
function renderMarkdown(md: string): string {
  ...
  for (const line of lines) {
    const t = line.trim();
    if      (t.startsWith('### ')) { if (inList) { html += '</ul>'; inList = false; } html += `<h3 style="font-size:13px;font-weight:700;margin:12px 0 5px">${inlineFmt(t.slice(4))}</h3>`; }
    else if (t.startsWith('## '))  { if (inList) { html += '</ul>'; inList = false; } html += `<h2 style="font-size:14px;font-weight:700;margin:14px 0 6px">${inlineFmt(t.slice(3))}</h2>`; }
    else if (t.startsWith('# '))   { if (inList) { html += '</ul>'; inList = false; } html += `<h1 style="font-size:16px;font-weight:700;margin:14px 0 7px">${inlineFmt(t.slice(2))}</h1>`; }
    else if (t.startsWith('- ') || t.startsWith('* ')) { if (!inList) { html += '<ul style="padding-left:18px;margin:6px 0">'; inList = true; } html += `<li style="margin-bottom:4px">${inlineFmt(t.slice(2))}</li>`; }
    else if (t === '') { if (inList) { html += '</ul>'; inList = false; } html += '<br>'; }
    else { if (inList) { html += '</ul>'; inList = false; } html += `<p style="margin:0 0 8px;line-height:1.65">${inlineFmt(t)}</p>`; }
  }
  if (inList) html += '</ul>';
  return html;
}
...
const NotificationsTab = React.forwardRef<NotificationsTabHandle, Props>(
  ...
  // NDA toolbar — insert Markdown syntax at textarea cursor
  ...
  return (
    <div>
      ...
      <div style={cardSt}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6, flexWrap: 'wrap' }}>
          {[
            ...
            <button key={btn.label} onClick={btn.action}
              style={{ padding: '4px 9px', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-overlay)', color: 'var(--text-secondary)', fontSize: 12, fontWeight: btn.bold ? 700 : 400, fontStyle: btn.italic ? 'italic' : 'normal', cursor: 'pointer', lineHeight: 1 }}>
              {btn.label}
            </button>
          ))}
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
            <button onClick={() => setShowPreview(false)}
              style={{ padding: '4px 10px', borderRadius: 4, border: `1px solid ${!showPreview ? 'var(--accent)' : 'var(--border)'}`, background: !showPreview ? 'var(--accent-dim)' : 'none', color: !showPreview ? 'var(--accent)' : 'var(--text-muted)', fontSize: 11, cursor: 'pointer' }}>
        ...
        {!showPreview ? (
          <textarea
            ...
            placeholder={t('tab6_nda_placeholder', '# Non-Disclosure Agreement\n\nAll information in this system is strictly confidential.')}
            rows={12}
      ...
      {/* ── 3. NDA ENFORCEMENT ──────────────────────────────────────────────── */}
      <div style={cardSt}>
        <SectionHeading color="var(--orange)" label={t('tab6_enforcement_section', 'NDA Enforcement')} />
        <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 14 }}>
          {t('tab6_enforcement_desc', 'Controls when users are required to re-accept the NDA.')}
        </div>

        <label style={{ display: 'flex', alignItems: 'flex-start', gap: 12, cursor: 'pointer' }}
          onClick={() => update('forceOnVersionChange', !form.forceOnVersionChange)}>
          <Toggle on={form.forceOnVersionChange} onChange={() => update('forceOnVersionChange', !form.forceOnVersionChange)} />
          <div>
            <div style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 500, marginBottom: 4 }}>
              {t('tab6_force_version_label', 'Force re-acceptance when project version changes')}
            </div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.6, maxWidth: 520 }}>
              {t('tab6_force_version_hint', 'When ON: users accept once per version. The NDA re-appears when the version in Project Identity is updated. When OFF: users accept at every login session.')}
            </div>
            {form.forceOnVersionChange && currentVersion && (
              <div style={{ marginTop: 8, padding: '6px 10px', background: 'var(--bg-overlay)', border: '1px solid var(--border)', borderRadius: 4, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{t('tab6_current_version', 'Current version')}:</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', fontFamily: "'DM Mono', monospace" }}>{currentVersion}</span>
              </div>
            )}
          </div>
        </label>
